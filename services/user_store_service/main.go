...
func main() {
	...
	defer func() {
		stop()
		wg.Wait()
		slog.Info("user-store-service stopped")
	}()

	slog.Info("user-store-service start", "environment", env.GetString("ENVIRONMENT", "dev"))
	wg.Go(func() {
		start(sctx, stop)
	})

	defer slog.Info("user-store-service shutting down...")
